...
import { ClipboardList, Search, Plus, Pencil, Trash2, X, Save, Loader2, Filter, ChevronDown } from 'lucide-react';
...
export default function ExamesProcedimentos() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api.get(`configuracoes/exames/?${params.toString()}`);
      setExames(res.data.results || []);
      setTotalPages(res.data.num_pages || Math.max(1, Math.ceil((res.data.count || 0) / (res.data.page_size || 1))));
    } catch (e) {
      notify.error("Erro ao carregar lista de exames.");
      setExames([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!formData.codigo_tuss.trim()) return notify.warning("Informe o codigo TUSS.");
    if (!formData.nome.trim()) return notify.warning("Informe o nome do exame.");

    setSaving(true);
    try {
      if (editingItem) {
        await api.put(`configuracoes/exames/${editingItem.id}/`, formData);
        notify.success('Exame atualizado com sucesso!');
      } else {
        await api.post('configuracoes/exames/', formData);
        notify.success('Novo exame cadastrado!');
      }
      setModalOpen(false);
      loadData();
    } catch (error) {
      notify.error("Erro ao salvar exame.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleInativar = async (item) => {
    ...
    const confirmado = await confirmDialog(
      "Deseja inativar este exame? Ele continuara visivel em registros antigos.",
      "Inativar Exame",
    ...
    try {
      await api.post(`configuracoes/exames/${item.id}/inativar/`);
      notify.success("Exame inativado.");
      loadData();
    } catch (e) {
      notify.error("Erro ao inativar exame.");
    }
  };

  const handleDelete = async (item) => {
    const confirmado = await confirmDialog(
      "Deseja excluir este exame permanentemente?",
      "Excluir Exame",
      "Confirmar Exclusao",
      "Cancelar",
      "danger"
    ...
    if (!confirmado) return;
    try {
      await api.delete(`configuracoes/exames/${item.id}/`);
      notify.success("Exame removido.");
      loadData();
    } catch (e) {
      const msg = e?.response?.data?.error || "Nao foi possivel excluir este exame.";
      notify.error(msg);
    }
  };
  ...
  const tipoOptions = [
    ...
    { value: 'Procedimento Clinico', label: 'Procedimento Clinico' },
    { value: 'Cirurgia/Intervencao', label: 'Cirurgia/Intervencao' }
  ];

  const filterTypeOptions = [
    { value: 'texto', label: 'Busca Geral' },
    { value: 'codigo_tuss', label: 'Codigo TUSS' },
  ...
  const addFilter = () => {
    ...
    if (filterType === 'tipo') {
      display = tipoOptions.find((o) => o.value === filterValue)?.label || filterValue;
    }
      ...
      <div className="max-w-6xl mx-auto pb-20">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Exames e Procedimentos</h1>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Cadastro oficial e importacao TUSS.</p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Codigo TUSS</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Nome</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Tipo</th>
                ...
                <>
                  {exames.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-700 dark:text-white">{item.codigo_tuss}</td>
                      <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{item.nome}</td>
                      <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{item.tipo}</td>
                  ...
                  {exames.length === 0 && (
                    <tr><td colSpan="5" className="p-10 text-center text-slate-400 font-medium italic">Nenhum exame encontrado.</td></tr>
                  )}
                </>
              )}
            </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 className="font-bold text-lg text-slate-800 dark:text-white">{editingItem ? 'Editar Exame' : 'Novo Exame'}</h3>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-5">
                <div>
                  <label className={labelClass}>Codigo TUSS</label>
                  <input
                    required
                    autoFocus
